<template>
  <div class="app-container">
        ...
        <el-row :gutter="16">
          <el-col :xl="6" :lg="6" :md="12" :sm="24" :xs="24">
            <el-form-item label="出库单号" prop="shipmentOrderNo">
              <el-input v-model="queryParams.shipmentOrderNo" clearable placeholder="请输入出库单号" @keyup.enter="handleQuery" />
            </el-form-item>
          </el-col>
          <el-col :xl="6" :lg="6" :md="12" :sm="24" :xs="24">
          ...
          <el-col :xl="6" :lg="6" :md="12" :sm="24" :xs="24">
            <el-form-item label="出库时间" prop="createTimeRange">
              <el-date-picker
    ...
    <el-card class="mt20">
      <el-table v-loading="loading" :data="detailList" border class="mt20" empty-text="暂无出库明细">
        <el-table-column label="出库单号" width="160">
          <template #default="{ row }">
            <el-button link type="primary" @click="handleGoOrder(row)" :disabled="!row.shipmentOrderId && !row.shipmentOrder?.id">
              {{ row.shipmentOrder?.shipmentOrderNo || '-' }}
            </el-button>
          </template>
        </el-table-column>
        ...
        <el-table-column label="仓库" width="110">
          <template #default="{ row }">
            {{ useWmsStore().warehouseMap.get(row.shipmentOrder?.warehouseId || row.warehouseId)?.warehouseName || '-' }}
          </template>
        </el-table-column>
        <el-table-column label="库区" width="110">
          <template #default="{ row }">
            {{ useWmsStore().areaMap.get(row.shipmentOrder?.areaId || row.areaId)?.areaName || '-' }}
          </template>
        </el-table-column>
...
<script setup name="ShipmentDetail">
...
function getList() {
  loading.value = true
  listShipmentOrderDetail(buildQuery()).then(response => {
...
const handleGoOrder = (row) => {
  const orderId = row.shipmentOrderId || row.shipmentOrder?.id
  if (!orderId) {
    proxy.$modal.msgWarning('当前记录缺少关联的出库单信息')
    return
  }
