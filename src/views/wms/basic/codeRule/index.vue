      ...
      <el-col :xl="17" :lg="16" :md="24" :sm="24" :xs="24">
        <el-card class="config-card" shadow="never">
          ...
          <div class="batch-header">
            <span class="batch-title">一键设置所有表单自动编码</span>
            <el-switch v-model="batchAutoCode" @change="handleBatchChange" />
          </div>
          ...
          >
            <!-- 开启自动编码 -->
            <el-form-item label="开启自动编码">
              <el-switch v-model="configForm.enabled" />
              <span class="form-tip" style="margin-left: 12px;">关闭后将使用手动录入编码</span>
            </el-form-item>
            ...
            <template v-if="configForm.enabled">
              ...
              <el-form-item label="前缀" prop="prefix">
                <el-input v-model="configForm.prefix" placeholder="请输入前缀，如 ZC" clearable maxlength="20" style="width: 200px" />
              </el-form-item>

              <!-- 后缀 -->
              <el-form-item label="后缀" prop="suffixType">
                <el-radio-group v-model="configForm.suffixType">
                  <el-radio label="ymd">年_月_日</el-radio>
                  <el-radio label="ym">年_月</el-radio>
...
const previewResult = computed(() => {
  const parts = []
  if (configForm.prefix) parts.push(configForm.prefix)

  const now = new Date()
  const sep = configForm.separator || ''
  if (configForm.suffixType === 'ymd') {
    parts.push(`${now.getFullYear()}${sep}${String(now.getMonth() + 1).padStart(2, '0')}${sep}${String(now.getDate()).padStart(2, '0')}`)
  } else if (configForm.suffixType === 'ym') {
    parts.push(`${now.getFullYear()}${sep}${String(now.getMonth() + 1).padStart(2, '0')}`)
  } else {
...
function handleBatchChange(val) {
  if (val && currentType.value) {
    proxy.$modal.msgSuccess('一键自动编码已开启，保存后生效')
  }
}
...
async function handleSave() {
  ...
  try {
    await updateCodeRule({
      id: configForm.id,
      enabled: configForm.enabled ? '0' : '1',
    ...
    if (item) {
      item.enabled = configForm.enabled
